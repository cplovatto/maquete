...
import { KpiCard, SortTh, fBRLR, fInt, fPct, groupByGerente, useSort } from './vdShared'
...
export default function RankingEquipesPage() {
  ...
  const keyOf = (e: VdEquipeRow, k: string): number => {
    switch (k) {
      ...
      case 'liquidas': return e.liquidas
  ...
  const groups = groupByGerente(equipes).map(g => ({
    gerente: g.gerente,
  ...
  return (
    <div className="page-content">
        ...
        <table className="dash-table">
          <thead>
            <tr>
              ...
              <SortTh label="Líquidas" sortKeyName="liquidas" active={sortKey === 'liquidas'} dir={sortDir} onSort={toggleSort} right />
                ...
                <Fragment key={g.gerente}>
                  {g.items.map(e => (
                    <tr key={e.id}>
                      ...
                      <td className="col-num" style={{ color: e.liquidas < 0 ? '#dc2626' : '#059669', fontWeight: 600 }}>
                        {e.liquidas > 0 ? '+' : ''}{fInt(e.liquidas)}
                      </td>
                  ...
                  <tr className="tfoot-total">
                    ...
                    <td className="col-num">{gLiquidas > 0 ? '+' : ''}{fInt(gLiquidas)}</td>
      ...
      <p className="page-subtitle" style={{ marginTop: 8 }}>
        Meta financeira média: {fBRLR(equipes.reduce((s, e) => s + e.metaFinanceira, 0) / equipes.length)} por equipe.
      </p>
    </div>
  )
}
