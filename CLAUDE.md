...
interface FileStatusCtxType {
  statuses: Record<string, FileStatus>       // estado de cada fonte de dados
  setStatuses: ...                           // setter direto (uso interno)
  onFileLoaded: (id: string) => void         // chame isto ao carregar um arquivo
  openImport: () => void                     // abre o modal de importação
  lastParcialUpload: Date | null             // timestamp do último upload parcial
  alertEnabled: boolean                      // alerta ativo ou não
  setAlertEnabled: ...
  ...
  toastVisible: boolean                      // controla visibilidade do toast
  setToastVisible: ...
}
```
...
Definidas como arrays de `DataSource` antes do `AppShell`:
...
- **Reset**: ao carregar um novo arquivo `parcial` via `onFileLoaded('parcial')`
...
| Componente | Função |
|---|---|
| `ImportModal` | Lista todas as fontes (abas Mensal/Anual), permite upload de arquivo por linha |
...
`.modal-overlay`, `.modal`, `.modal--sm`, `.modal-header`, `.modal-title`, `.modal-close`, `.modal-tabs`, `.modal-tab.active`, `.modal-body`, `.import-section-title`, `.import-row`, `.import-icon`, `.import-name`, `.import-status.ok`, `.import-format-badge`, `.format-xlsx`, `.format-csv`
...
- Usar PRs para toda mudança — PR #1 (sidebar) já mergeado em `main`
- Branch ativo no momento da escrita deste documento: `feature/profile-import-modal`
- Ferramenta: `gh` CLI instalada em `C:\Program Files\GitHub CLI\gh.exe`
