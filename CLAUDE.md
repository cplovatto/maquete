...
- **RPA e UPA** (só no Desempenho do ER, `ErPage`/`ErEquipesModal` modo desempenho): `rpaValor` (equivalente ao "Boleto Médio" do Canal Loja, R$) e `upaValor` (equivalente a "Itens por Venda"), ambos em `VdEquipeRow`. Meta editável em nível de rede via `useVdDesempenhoMetas()` (`prisma-prefs-vd-desempenho-metas`, defaults RPA R$120 / UPA 1,8), célula colorida vs. meta via `ValueMetaCell` (variante de `MetaCell` pra valores absolutos, não percentuais). No agregado por ER, `agregarPorEr()` usa a média das equipes (não soma, já que é um valor médio por venda).
